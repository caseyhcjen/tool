...
def data_interpolate_panda(s):
    #http://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.interpolate.html
    #http://pandas.pydata.org/pandas-docs/stable/generated/pandas.DataFrame.fillna.html
    #Interpolate values using Panda
    p = pd.Series(s)
    #p.interpolate(method='nearest')
    p.interpolate(method='linear')
    p=p.interpolate()
    s_new=p.values
    return s_new

def data_Imputer_sklearn(s):
    imp = Imputer(missing_values=np.nan, strategy='mean', axis=1)
    imp.fit(x)
    s_new=imp.transform(s)
    return s_new[0]

def downsample(s, p, phase=0):
    """Decrease sampling rate by integer factor n with included offset phase.
    """
    if int(p)==p:
        #which means we have a integer times of newsamplerate
        #therefore, we can downsize to take one point for every p point
        return s[phase::int(p)]
    else:
        return interpolationsample(s,1./p)

def upsample(s, p):
    """Increase sampling rate by integer factor n  with included offset phase.
       Casey 2014/08/21
       Revised with scipy.ndimage interpolation
    """
    return interpolationsample(s,p)

def interpolationsample(s, n, phase=0):
    """Casey 2014/08/21
       Revised with scipy.ndimage interpolation
    """
    import scipy.ndimage
    #return numpy.roll(numpy.kron(s, numpy.r_[1, numpy.zeros(n-1)]), phase)
    #return numpy.round(scipy.ndimage.interpolation.zoom(s, n), 1)
    return numpy.round(scipy.ndimage.interpolation.zoom(s, n, order=1), 1)
